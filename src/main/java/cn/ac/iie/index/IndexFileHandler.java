...
import cn.ac.iie.util.ByteBufferUtils;
                        ...
                        if (tk < (long) range.right.getTokenValue()) {
                            ...
                            if (index.containsKey(key)) {
                                ...
                                if (errIndex.containsKey(key)) {
                                    ...
                                    errIndex.put(key, new ArrayList<String>() {{
                                        add(uuid);
                                        add(fpath);
                                        add(ast + "");
                                        add(aend + "");
...
//        });
    ...
    public static void findDocFromIndexFile(String indexfile, String ks, String tb, String pk, String... ck) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException, IOException {
        DatabaseDescriptor.daemonInitialization();
        Schema.instance.loadFromDisk(false);
        Keyspace.setInitialized();
        Keyspace.open("system");
        ...
        ByteBuffer pkb = ByteBufferUtil.bytes(pk);
        DecoratedKey dk = new BufferDecoratedKey(Murmur3Partitioner.instance.getToken(pkb), pkb);
        Term term = null;
        ...
        if (cfm.clusteringColumns().size()> 0) {
            ...
            method.setAccessible(true);
            BytesRef bytesef=(BytesRef) method.invoke(keyMapper,dk,clustering);
            term=new Term("_key",bytesef);
        } else {
            keyMapper = new KeyMapper(cfs.metadata);
            term = keyMapper.term(dk, null);
        }


        DirectoryReader reader = DirectoryReader.open(FSDirectory.open(Paths.get(indexfile)));
        IndexSearcher searcher=new IndexSearcher(reader);
        TopDocs topDocs=searcher.search(new TermQuery(term),100);

        for (int i = 0; i < topDocs.scoreDocs.length; i++) {

            ScoreDoc scoreDoc = topDocs.scoreDocs[i];
            int no = scoreDoc.doc;
            Document document = searcher.doc(no);
    ...
    public static void deleteDocFromIndexFile(String indexfile, String ks, String tb, String pk, String... ck) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException, IOException {
        DatabaseDescriptor.daemonInitialization();
        Schema.instance.loadFromDisk(false);
        Keyspace.setInitialized();
        Keyspace.open("system");
        ...
        IndexWriterConfig iwc = new IndexWriterConfig();
        iwc.setOpenMode(IndexWriterConfig.OpenMode.CREATE_OR_APPEND);
        IndexWriter baseWriter = new IndexWriter(FSDirectory.open(new File(indexfile).toPath()), iwc);
        baseWriter.deleteDocuments(term);
        reader.close();
        baseWriter.close();
    }
}
